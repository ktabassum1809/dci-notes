  ...
  it("Inputs should have corresponding label tags", async () => {
    try {
      const labels = await page.$$eval(
        ...
        (els) =>
          els.filter((el) => document.querySelector(`label[for="${el.id}"]`))
      );
      console.log(labels);
...
describe("Form Styling", () => {
  it("Input fields with type='text' attribute Should have a hotpink color outline styling focus", async () => {
    try {
      ...
      const textInputsColor = await Promise.all(
  ...
  it("'Email' & 'textarea' Input fields should have blue color outline styling on focus", async () => {
    try {
      ...
      await input.focus();
      const inputColor = await page.$eval(
        'input[type="email"]',
      ...
      const textareaColor = await page.$eval(
        "textarea",
        (el) => getComputedStyle(el).outline
      );
