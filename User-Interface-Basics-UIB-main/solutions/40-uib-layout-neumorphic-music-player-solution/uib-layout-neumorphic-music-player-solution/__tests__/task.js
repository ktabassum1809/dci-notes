...
describe("Flexbox", () => {
  it("Page should be styled with flexbox", async () => {
    const flexBox = await page.$$eval("*", (el) =>
...
describe("Fonts and Icons", () => {
  it("Font Awesome icons should be present on page", async () => {
    const fonts = await page.$$eval("*", (el) =>
      Array.from(el).map((e) =>
        getComputedStyle(e).getPropertyValue("font-family")
      )
    );
    expect(fonts.some((f) => f.match(/Font Awesome/)));
  });
  ...
  it("Page should be styled with different 'font-weights'", async () => {
    const fontWeights = await page.$$eval("*", (el) =>
      Array.from(el).map((e) =>
        getComputedStyle(e).getPropertyValue("font-weight")
      )
    );
